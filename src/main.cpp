...
int main() {
    ...
    // bgr
    auto img_white = Segment::to_mono(img, cv::Vec3b(230, 230, 230), cv::Vec3b(250, 250, 240));
    DEBUG(Utils::show(img_white);)
    auto img_white_open = Morphology::open(img_white);
    DEBUG(Utils::show(img_white_open);)

    auto img_yellow = Segment::to_mono(img, cv::Vec3b(0, 200, 240), cv::Vec3b(100, 230, 255));
